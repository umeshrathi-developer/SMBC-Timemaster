...
from timesheet.utils import import_client_timesheet_file
...
class ClientTimesheetImportTests(TestCase):
    ...
    def _import_workbook(self, rows):
        workbook = Workbook()
        ...
        try:
            ...
            return import_client_timesheet_file(
                temp_file.name,
    ...
    def test_client_import_matches_name_before_header_line_break_id(self):
        result = self._import_workbook([
            ['Ignored title'],
            ['Date', 'Alice Smith\n8619'],
            [date(2026, 4, 1), 8],
        ])
    ...
    def test_client_import_updates_existing_draft_for_employee_date(self):
        TimesheetEntry.objects.create(
            employee=self.employee,
            date=date(2026, 4, 1),
            project='Old Uploaded Project',
            hours=4,
            status='DRAFT',
        )

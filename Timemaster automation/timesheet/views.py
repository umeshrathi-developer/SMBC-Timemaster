...
def register(request):
    """User registration - automatically adds users to Employee group"""
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Automatically add new users to Employee group
            try:
                employee_group, created = Group.objects.get_or_create(name='Employee')
                user.groups.add(employee_group)
                access_logger.info(f"New user registered and added to Employee group: {user.username}")
            except Exception as e:
                access_logger.warning(f"Error adding user to Employee group: {str(e)}")
                logger.warning(f"Error adding user to Employee group: {str(e)}")
...
def login_view(request):
    ...
    if request.method == 'POST':
        ...
        if user is not None:
            # Check if user has permission to access employee features
            if not can_access_employee_features(user):
                access_logger.warning(f"Login failed for {username}: User not in Employee or Admin group")
                messages.error(request, 'Your account has not been activated. Please contact an administrator.')
...
def dashboard(request):
    ...
    if admin:
        ...
        if not employee:
            # Show a pending setup message instead of error
            messages.warning(request, 'Your employee profile is still being set up. Please contact an administrator to link your account.')
            context = {
                'total_employees': 0,
                'total_compoffs': 0,
                'pending_compoffs': 0,
...
@login_required(login_url='login')
def import_client_timesheet_data(request):
    """Import client timesheet data and create TimesheetEntry records in DRAFT status - Admin only
    ...
    - Only creates entries for cells with value > 0
    """
    if not is_admin(request.user):
        access_logger.warning(f"Unauthorized access attempt to import_client_timesheet_data by user: {request.user.username}")
        messages.error(request, 'You do not have permission to access this page.')
        return redirect('dashboard')
    
    import tempfile
    import os
    from .forms import ClientTimesheetImportForm
    from .utils import import_client_timesheet_file
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            try:
                ...
                # Import data
                try:
                    result = import_client_timesheet_file(temp_file.name, import_date=import_date, project_name=project_name)
                    last_import_result = result

                    report_path = None
                    if result.get('skipped_details') or result.get('failed_details'):
                        import csv

                        report_dir = os.path.join(settings.BASE_DIR, 'import_reports')
                        os.makedirs(report_dir, exist_ok=True)
                        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
                        report_path = os.path.join(report_dir, f'client_timesheet_import_{timestamp}.csv')

                        with open(report_path, 'w', newline='', encoding='utf-8') as report_file:
                            fieldnames = [
                                'outcome', 'row', 'column', 'date', 'employee',
                                'raw_header', 'value', 'reason', 'project',
                            ]
                            writer = csv.DictWriter(report_file, fieldnames=fieldnames)
                            writer.writeheader()

                            for detail in result.get('failed_details', []):
                                writer.writerow({'outcome': 'failed', **detail})
                            for detail in result.get('skipped_details', []):
                                writer.writerow({'outcome': 'skipped', **detail})

                        result['report_path'] = report_path
                        result['report_relative_path'] = os.path.relpath(report_path, settings.BASE_DIR)
                        import_logger.info(f"Client timesheet import issue report written to: {report_path}")
                    
                    if result['success']:
                        import_logger.info(f"Client timesheet import successful by user {request.user.username}: "
                                          f"Created={result['entries_created']}, Updated={result['entries_updated']}, "
                                          f"Skipped={result['entries_skipped']}, Project={project_name}")
                        ...
                        if report_path:
                            messages.warning(
                                request,
                                f"Skipped/failed detail report created: {result['report_relative_path']}"
                            )
                    else:
                        import_logger.error(f"Client timesheet import failed by user {request.user.username}: {result['errors']}")
                        messages.error(request, f"{result['message']}\n" + "\n".join(result['errors'][:5]))
                        if report_path:
                            messages.warning(
                                request,
                    ...
                    import_logger.error(f"Error during client timesheet import for user {request.user.username}: {str(e)}", exc_info=True)
                    logger.error(f"Client timesheet import error: {str(e)}")
                    messages.error(request, f"Error during import: {str(e)}")
            except Exception as e:
                import_logger.error(f"Error processing client timesheet file for user {request.user.username}: {str(e)}", exc_info=True)
                logger.error(f"File processing error: {str(e)}")
                messages.error(request, f"Error processing file: {str(e)}")
            finally:
                # Clean up temp file
    ...
    return render(request, 'timesheet/import_client_timesheet.html', context)
